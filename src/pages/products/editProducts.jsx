...
const Productedit = () => {
  ...
  const getProductedit = async () => {
    let proeditdetails = await fetch(config.apiurl + "/products/" + params.id, {
      method: "get",
      headers: {
  ...
  const getSubCategory = async () => {
    let subcatresult = await fetch(
      config.apiurl + `/subcategory/${mainCategoryParam}`,
      {
        method: "get",
        headers: {
          Authorization: "bearer " + accesstoken.data.access_token,
  ...
  const submitProductDetails = async (res) => {
    const editupdateurl = config.apiurl + "/products/" + params.id;
    const configdata = {
      headers: {
